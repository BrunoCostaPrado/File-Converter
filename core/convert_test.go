...
func TestBuildFfmpegArgsHWAccelBitrate(t *testing.T) {
	p := Preset{
		Container: "mp4", VideoCodec: "h264", AudioCodec: "aac",
		Quality: 23, Preset: "medium", HWAccel: "nvenc",
	}
	args := BuildFfmpegArgs("in.mp4", "out.mp4", p, "2M")
	// bitrate replaces -crf for HW encoders
	expected := []string{"-i", "in.mp4", "-c:v", "h264_nvenc", "-b:v", "2M", "-c:a", "aac", "out.mp4"}
	if len(args) != len(expected) {
		t.Fatalf("expected %d args, got %d: %v", len(expected), len(args), args)
	}
	for i := range expected {
